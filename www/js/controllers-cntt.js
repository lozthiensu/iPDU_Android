...
.controller('page_Cntt_Ctrl', function ($scope, pduService, $rootScope, $timeout, localStorageService, $cordovaSQLite, $cordovaFileTransfer, $cordovaFile, $cordovaSocialSharing, $cordovaStatusbar, $cordovaInAppBrowser, $cordovaProgress, $cordovaSpinnerDialog, $cordovaDialogs) {
    ...
    $scope.deleteToSQLite = function (idBaiViet) {
        ...
        for (i = 0; i < tongSoBai; i++) {
            if ($scope.tempListId[i].Id == idBaiViet.Id + 'cntt') {
    ...
    $scope.checkTonTai = function (idBaiViet) {
        ...
        for (i = 0; i < n; i++) {
            if (idBaiViet.Id + 'cntt' == $scope.tempListId[i].Id) {
                $scope.tonTai = true;
                break;
            }
        }
    ...
    $scope.theLoaiList =[{name: "Tất cả", id:"all"},{name: "Thông báo", id:"thongbao"},{name: "Thời khóa biểu", id:"tkb"},{name: "Học sinh sinh viên", id:"hssv"},{name: "Lịch thi", id:"bangdiem"},{name: "Đoàn TN", id:"doantn"},{name: "Olympic/ACM ICPC", id:"olympic"}];
    ...
    $scope.getDataFromSever = function () {
        if ($scope.noMoreItemsAvailable == false) {
            pduService.Cntt_getPage(pageIndex, theLoai).success(function (data) {
                n = data.length, i = 0;
                if (n < 1)
                    $scope.noMoreItemsAvailable = true;
                for (; i < n; i++)
                    $scope.dataListCntt.push(data[i]);
            });
            pageIndex++;
        }
    };
